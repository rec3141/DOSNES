...
function(X, initial_config = NULL, k=3, initial_dims=30, perplexity=30, max_iter = 1000, min_cost=0, epoch_callback=NULL,whiten=TRUE, epoch=100 ){
	...
	for (iter in 1:max_iter){
        ...
        #spherical embedding
        # matlab: ydata = bsxfun(@minus, ydata, mean(ydata, 1));
        # center columns by substracting column means from each column
        ydata = sweep(ydata, 2, colMeans(ydata), "-") #sweep(...,2,...) is row-wise

        # matlab: r_mean = mean(sqrt(sum(ydata.^2,2)),1);
        r_mean = mean(sqrt(rowSums(ydata^2)))
        
        # matlab: ydata = bsxfun(@times, ydata, r_mean./ sqrt(sum(ydata.^2,2)) );
        ydata = sweep(ydata, 1, r_mean/sqrt(rowSums(ydata^2)), "*")
